...
public abstract class S3Base implements AutoCloseable {
  ...
  private Request newRequest(
      ...
      throws InsufficientDataException, InternalException, IOException, NoSuchAlgorithmException,
          XmlParserException, InvalidKeyException {
    if (contentType == null) contentType = Http.DEFAULT_MEDIA_TYPE;

    Checksum.Algorithm defaultAlgorithm = null;
    if (creds == null) defaultAlgorithm = Checksum.Algorithm.MD5;
    if (!url.isHttps()) defaultAlgorithm = Checksum.Algorithm.SHA256;

    String sha256Hash = defaultAlgorithm == null ? "UNSIGNED-PAYLOAD" : null;
    String md5Hash = null;

    PartSource partSource = null;
    byte[] data = null;

    boolean traceBody = false; // Return traceBody

    if (body != null) {
      if (body instanceof okhttp3.RequestBody) {
        // Special case: Create a new request any further checking.
        return Http.newRequest(
            url, method, headers, new Http.RequestBody((okhttp3.RequestBody) body), this.userAgent);
      } else if (body instanceof PartSource) {
        // Do additional checking for PartSource
        partSource = (PartSource) body;
        checksums = partSource.checksums();
        if (checksums == null) {
          throw new IllegalArgumentException("checksums must be provided via PartSource");
        } else if (!checksums.containsKey(defaultAlgorithm)) {
          throw new IllegalArgumentException(
              String.format(
                  "default checksum %s must be provided via PartSource", defaultAlgorithm));
        }
        byte[] sum = checksums.get(defaultAlgorithm);
        if (sum == null) {
          throw new IllegalArgumentException(
              String.format(
                  "default checksum %s value must be provided via PartSource", defaultAlgorithm));
        }
        if (defaultAlgorithm == Checksum.Algorithm.MD5) {
          md5Hash = Checksum.base64String(sum);
        } else if (defaultAlgorithm == Checksum.Algorithm.SHA256) {
          sha256Hash = Checksum.hexString(sum);
        }
        ...
        if (body instanceof CharSequence) {
          ...
          contentType = Http.XML_MEDIA_TYPE;
        }
        length = data.length;
        traceBody = true;
      }
      ...
      if (data != null) {
        byte[] sum =
            (checksums != null && defaultAlgorithm != null)
                ? checksums.get(defaultAlgorithm)
                : null;
        if (defaultAlgorithm == Checksum.Algorithm.MD5) {
          if (sum == null) sum = Checksum.MD5.sum(data);
          md5Hash = Checksum.base64String(sum);
        } else if (defaultAlgorithm == Checksum.Algorithm.SHA256) {
          if (sum == null) sum = Checksum.SHA256.sum(data);
          sha256Hash = Checksum.hexString(sum);
        }
      }
      ...
      if (checksums != null) {
        for (Map.Entry<Checksum.Algorithm, byte[]> entry : checksums.entrySet()) {
          if (entry.getKey() != null && entry.getValue() != null) {
            builder.add(entry.getKey().header(), Checksum.base64String(entry.getValue()));
          }
        }
      }
    ...
    if (data != null) {
      requestBody = new Http.RequestBody(data, length, contentType);
    } else {
      requestBody = new Http.RequestBody(partSource.inputStream(), partSource.size(), contentType);
    }
    Request request = Http.newRequest(url, method, headers, requestBody, userAgent);
  ...
  protected CompletableFuture<Response> executeAsync(
    ...
    if (body == null && (method == Http.Method.PUT || method == Http.Method.POST)) {
